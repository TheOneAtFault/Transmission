...
public class ESCUtil {
    ...
    /**
     * Set a single-byte character set
     */
    ...
    private static byte[] getBytesForPrintQRCode(boolean single) {
        //Print the QR code of the saved data
        byte[] dtmp;
        if (single) {        //Only one QRCode is printed on the same line, followed by a newline
            dtmp = new byte[9];
            dtmp[8] = 0x0A;
        } else {
            dtmp = new byte[8];
